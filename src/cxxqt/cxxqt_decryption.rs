...
impl qobject::QtDecryption {
    pub unsafe fn decrypt(mut self: Pin<&mut Self>, state: *const QtState) {
        ...
        let mut data = if self.rust().input_type == 0 {
            let hex_str = self.rust().input_text.to_string();
            match hex::decode(hex_str) {
                Ok(v) => v,
                Err(_) => {
                    self.as_mut().set_output_error(QString::from("The input is not valid hex!"));
                    return;
                }
            }
        } else {
        ...
        if self.rust().output_type == 0 {
            match String::from_utf8(data) {
                Ok(text) => self.as_mut().set_output_text(QString::from(&text)),
                Err(_) => self.as_mut().set_output_error(QString::from(
                    "Decryption succeeded, but output is not valid UTF-8 text."
                )),
            }
        } else {
...
pub mod qobject {
    ...
    extern "RustQt" {
        ...
        #[namespace = "qt_decryption"]
        type QtDecryption = super::RDecryption;

        #[qinvokable]
        #[cxx_name = "decrypt"]
        unsafe fn decrypt(self: Pin<&mut QtDecryption>, state: *const QtState);

        #[qinvokable]
        #[cxx_name = "aes_random_key"]
        unsafe fn aes_random_key(self: Pin<&mut QtDecryption>, state: *const QtState);

        #[qinvokable]
        #[cxx_name = "aes_random_iv"]
        unsafe fn aes_random_iv(self: Pin<&mut QtDecryption>, state: *const QtState);
    }
}